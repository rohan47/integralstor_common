  ...
  try:
    ...
    with open('%s/config/%s.conf'%(root,service),'w') as f:
      ...
      if service == 'clamd':
        ...
        os.symlink('%s/config/%s.conf'%(root,service),'/etc/clamd.d/scan.conf')
        service = 'clamd@scan'
        action = 'restart'
        ...
        f.write('OnOutdatedExecute %s/av_event_update_available.py "%%v"\n'%script_root)
        if os.path.isfile('/etc/freshclam.conf'):
          os.unlink('/etc/freshclam.conf')
        os.symlink('%s/config/%s.conf'%(root,service),'/etc/freshclam.conf')
